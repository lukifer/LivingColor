...
var LivingColor = {
	...
	"start": function(opts)
	{
		...
		// Shrink site content down to half-width
		$(LivingColor.options.wrapper).css("width", "50%");
		...
		// Attach events
		$(document).on("click", "#colorRulesAdd", LivingColor.addRule);
		$(document).on("change", "#colorRules input[type=text]", LivingColor.inputChange);
		$(document).on("change", "#colorRulesList", LivingColor.colorListChange);
		$(document).on("click", "#colorRulesListSave", LivingColor.saveNewColorList);

		// Trigger the starting rules
		$("#colorRules input[type=text]").change();
	},

	"loadRulesList": function(name)
	{
		var theRulesList = JSON.parse(localStorage.getItem("colorRules") || "false");
		if(!theRulesList || !theRulesList.names)
		{
			theRulesList = {"names": ["Default"], "active": "Default"};
			localStorage.setItem("colorRules", JSON.stringify(theRulesList));
		}
	...
	{
		var theRules = localStorage.getItem("colorRules"+name);
		...
		else
		{
			...
			localStorage.setItem("colorRules"+name, JSON.stringify(theRules));
		}
	...
	"liveColorChange": function(elem)
	{
		...
		if(elem != $bgc[0]) $(selector).css("color", color ? "#"+color : "");
		if(elem != $col[0]) $(selector).css("backgroundColor", bgColor ? "#"+bgColor : "");
	},
	...
	"addRule": function()
	{
		...
		$("#colorRules ul").append(LivingColor.renderOne(newRule));
	...
	{			
		...
		$("#colorRules li").each(function(n, li)
		{
		...
		localStorage.setItem("colorRules"+LivingColor.rulesList.active, JSON.stringify(LivingColor.rules));
	},
	
	"colorListChange": function()
	{
		var newActive = $("#colorRulesList").val();
		if(!newActive) return;
	...
	"saveNewColorList": function()
	{
		...
		localStorage.setItem("colorRules", JSON.stringify(LivingColor.rulesList));
		localStorage.setItem("colorRules"+newName, JSON.stringify(LivingColor.rules));
	...
	"render": function()
	{
		...
		{
			$("body").append('<div id="colorRules" style="position: absolute; top: 0; right: 0; width: 50%; bottom: 0; z-index: 1000; background-color: #222;"></div>');
			$colorRules = $("#colorRules");
		}
		
		// Render add button
		var html = '<ul></ul><div><input type="button" id="colorRulesAdd" value="Add Rule" style="float:right;" />';

		// Render select menu for color schemes
		var activeName = LivingColor.rulesList.active;
		html += '<select id="colorRulesList">';
		$.each(LivingColor.rulesList.names, function(n, name)
		{
			html += '<option value="'+name+'"'+(activeName == name ? ' selected="selected"' : '')+'>'+name+'</option>';
		});
		html += '</select>';
		html += '<input type="button" id="colorRulesListSave" value="Save...">';
		html += '</div>';
		$colorRules.html(html);
		...
		$colorRules.children("ul").html(html);
	...
	{
		return '<li>'
		+	'<input type="text" name="selector" class="selector" value="'+rule.selector+'" placeholder="Selector (#id .class)" style="margin-left: 3px; width: 48%;" />'
		+	'<input type="text" name="color" class="textColor color {required:false,onImmediateChange:\'LivingColor.liveColorChange(this.valueElement)\'}" value="'+rule.color+'" placeholder="Text" style="width: 24%; margin-left: 3px;" />'
		+	'<input type="text" name="backgroundColor" class="backgroundColor color {required:false,onImmediateChange:\'LivingColor.liveColorChange(this.valueElement)\'}" value="'+rule.backgroundColor+'" placeholder="Background" style="width: 24%; margin-left: 3px;" />'
		+	'</li>';
	}
};
